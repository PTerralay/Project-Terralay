...
;
;Desc:
; This is the object that is the world. A world contains maps,
; agents and the player.
; A world also has a state-variable wich tells us what has happened in the world
; and what has yet to happen.
; we also load the graphics to the world so that we have them predefined,
; this helps the game run smoother.
;_______________________________________________________________________________
  ...
  (class object%
    ...
    (field (chars '())
           ...
           (player (instantiate Player% 
                     (96 96 3 3 'right this canvas 8 (new Inventory%
                                                          (width 5)
                                                          (height 3))))))
    ...
    (define/public (draw-text-ingame map x y size string ticks)
      (set-box! message-list-box (mcons (mcons ticks
                                               (list map x y size string))
    ...
    (define/public (add-neighbours! map)
      ...
      (for-each
       (lambda (element)
         (display (car element))
         (display " ")
         (add-map! (load&create-map (car element)
                                    (cadr element)
                                    this)))
    ...
    ;---------------------------------------------------------------------------
    ; This is how we move between the maps,
    ; set the current map to chosen neighbour,
    ; then we move the player to the place where the door is in the new map
    ; and make him face away from the exit.
    ; If the player is chased, the monsters chasing him will eventually
    ; be moved to the new room as well.
    ;params:
    ; mapname - the name of the new map
                ...
                [notify-callback 
                 (lambda ()
                   (when (and (eqv? (get-field mapID current-map) mapname)
                     ...
                     (set-field! place char mapname)))]
                [interval (+ (sqr (- (get-field gridx player)
                                     (get-field gridx char)))
                             (sqr (- (get-field gridy player)
    ...
    ;---------------------------------------------------------------------------
    ;The characters are simply instansiated and saved
    ; to the list of characters and update agents.
    ...
    ;---------------------------------------------------------------------------
    ;things-load:
    ; loads the things into the world
    ;params:
    ; thing-list - a list of names of the things that are
    ; to be loaded from the Agentdata file.
    ;---------------------------------------------------------------------------
    (define/public (things-load datalist)
      (define (load-loop thing-list)
        (if (null? thing-list)
            '()
            (let* ((data (dynamic-require
                          "Gamedata/Agentdata.rkt"
                          (car thing-list)))
                   (new-thing
                    (new Thing%
    ...
    ;---------------------------------------------------------------------------
    ;savegame:
    ; writes Data to a file with the name filename, where Data is a number of
    ; associative lists within an associative list
    ; containing all relevant information about the game that needs to be saved.
    ;params:
    ...
    (define/public (loadgame filename)
      (let* ((loadfile (open-input-file filename #:mode 'binary))
        ...
        (define (loop loadlist)
          (if (null? loadlist)
              ...
              (let ((element (car loadlist)))
                (case (car element)
                  ((world)
                   (set! state (cdr (assq 'state (cadr element))))
                   (let ((map-pair
                          (assq (cdr (assq 'current-map (cadr element)))
                                (dynamic-require
                                 "Gamedata/MapIndex.rkt"
                                 'Index))))
                     (set! current-map (load&create-map
                                        (car map-pair)
                                        (cdr map-pair) this))
                     (add-neighbours! current-map)))
                  ((monster)
                   (let* ((monsterfile
                           (cdr (assq (cdr (assq 'name (cadr element)))
                                      (dynamic-require
                                       "Gamedata/Agentdata.rkt"
                                       'Character-list)))))
                     ...
                     (set! chars
                           (mcons
                            (new Character%
                                 (gridx
                                  (cdr (assq 'gridx (cadr element))))
                                 (gridy
                                  (cdr (assq 'gridy (cadr element))))
                                 (triggerlist
                                  (dynamic-require monsterfile 'triggers))
                                 (move-cond
                                  (dynamic-require monsterfile 'movecondition))
                                 (interaction
                                  (dynamic-require monsterfile 'interact-code))
                                 (agent-ID
                                  (cdr (assq 'name (cadr element))))
                                 (world
                                  this)
                                 (tex-Width
                                  (dynamic-require monsterfile 'tex-Width))
                                 (passable
                                  (dynamic-require monsterfile 'passable?))
                                 (tex-Height
                                  (dynamic-require monsterfile 'tex-Height))
                                 (tex-ID
                                  (dynamic-require monsterfile 'tex-ID))
                                 (place
                                  (cdr (assq 'place (cadr element))))
                                 (state
                                  (cdr (assq 'state (cadr element))))
                                 (speed
                                  (dynamic-require monsterfile 'speed)))
                            chars))))
                  
                  ((thing)
                   (let* ((thingdata (dynamic-require
                                      "Gamedata/Agentdata.rkt"
                                      (cdr (assq 'name (cadr element)))))
                          ...
                          (new-thing
                           (new Thing%
                                ...
                                (gridy
                                 (cdr (assq 'gridy (cadr element))))
                                (triggerlist
                                 (cdr (assq 'triggers thingdata)))
                                (interaction
                                 (cdr (assq 'interaction-code thingdata)))
                                (world
                                 this)
                                (tex-ID
                                 (cdr (assq 'tex-ID thingdata)))
                                (tex-width
                                 (cdr (assq 'tex-width thingdata)))
                                (tex-height
                                 (cdr (assq 'tex-height thingdata)))
                                (tex-rel-x
                                 (cdr (assq 'tex-rel-x thingdata)))
                                (tex-rel-y
                                 (cdr (assq 'tex-rel-y thingdata)))
                                (passable
                                 (cdr (assq 'passable? thingdata)))
                                (agent-ID
                                 (cdr (assq 'name (cadr element))))
                                (inv-name
                                 (cdr (assq 'inv-name thingdata)))
                                (place
                                 (cdr (assq 'place (cadr element))))
                                (state 
                                 (cdr (assq 'state (cadr element)))))))
        ...
        (set! agents '())
        ;the message-list is mutable and therefore difficult to save.
        ;we clear it so that no messages will stay behind after the load.
