...
;-------------------------------------------------------------------------------
; setup for how the main-menu and it's submenus will be structured.
;-------------------------------------------------------------------------------
(define (setup-main-menu world)
    ...
    (list (list 
          ...
          (list
           ...
           (cons 'fn (lambda (menu)
                       ...
                       (set-field! game-start-ticker world 0)
                       (send (get-field parent menu) leave-menu!))))
          (list
           (cons 'text "Load Game")
           (cons 'fn (lambda (menu)
                       ...
                       (set-field! state
                                   (list-ref (get-field children menu) 0) 0))))
          (list
           (cons 'text "Save Game")
           (cons 'fn (lambda (menu)
                       (set-field! state menu -1)
                       (set-field! state
                                   (list-ref (get-field children menu) 1) 0))))
          (list
           (cons 'text "Exit")
           (cons 'fn (lambda (menu)
                       (exit))))))
           ...
           (list
            ...
            (cons 'fn
                  (λ (menu)
                    ...
                    (send (get-field parent (get-field parent menu))
                          leave-menu!))))))
         (children'())))
  ...
  (set-field! children main-menu (list loadmenu savemenu))
