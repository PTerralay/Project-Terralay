...
(define Inventory%
  (class object%
    ...
    ;---------------------------------------------------------------------------
    ;gridloop:
    ; A short looping method to initialize the inventory grid with unique rows.
    ;We must use this approach since initializing grid to
    ; (make-vector height (make-vector width #f)) creates 
    ; a grid with the _SAME_ object in all the rows,
    ; and mutating one will mutate all the others.
    ;params:
    ...
    ;---------------------------------------------------------------------------
    ;update-inventory:
    ; Updates the inventory grid with data from the inventory's list of things.
    ;---------------------------------------------------------------------------
    (define/public (update-inventory world)
      (let ((things '()))
    ...
    ;params:
    ; direction - the direction in which to move the cursor, or another command.
    ;---------------------------------------------------------------------------
