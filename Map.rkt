...
(define (map-load data-file)
  ;Don't forget that maps are loaded both in the initial load&create-map in Game,
  ; but also when setting the current map and loading the neighbours
  (let ((iy 0)
        (y-vector '()))    
    (define (y-loop)
      (let ((ix 0)
