...
(define (texloop filename i number-of-textures texwidth texheight)
  (if (eq? i number-of-textures)
      '()
      (cons (bitmaparea->gl-vector (make-object bitmap% filename 'png/alpha #f)
                                   (* i texwidth) 0 texwidth texheight)
            (texloop filename (+ i 1) number-of-textures texwidth texheight))))
   ...
   (lambda (tile-list)
     (if (eq? (length tile-list) 16)
         (for-each
          (λ (tex)
            (glBindTexture
             ...
             (gl-vector-ref tile-texture-list (+ (* i 16) j)))
         ...
         (begin
           (glBindTexture
            GL_TEXTURE_2D
            (gl-vector-ref tile-texture-list (* i 16)))
     ...
     (if (eq? (length char-list) 20)
         (for-each
          (lambda (tex)
            ...
            (glBindTexture
             GL_TEXTURE_2D
             (gl-vector-ref char-texture-list (+ (* i 20) j)))
         ...
         (begin
           (glBindTexture
            GL_TEXTURE_2D
            (gl-vector-ref char-texture-list (* i 20)))
...
(let ((i 0))
  (for-each
   (lambda (thing-list)
     (glBindTexture
      GL_TEXTURE_2D
      (gl-vector-ref thing-texture-list i))
  ...
  (letrec ((loop
            (λ (i)
              (cond ((> i 69)
                     '())
                    ((> i 34)
                     (cons (bitmaparea->gl-vector alphabetbitmap
                                                  (+ (* (- i 35) 18) 2)33 18 31)
                           (loop (+ i 1))))
                    (else (cons (bitmaparea->gl-vector alphabetbitmap
                                                       (+ (* i 18) 2) 0 18 31)
                                (loop (+ i 1))))))))
...
(letrec ((loop
          (λ (i)
            (when (< i 70)
              (glBindTexture
               GL_TEXTURE_2D
               (gl-vector-ref text-texture-list i))
              ...
              (glTexImage2D
               ...
               0
               4
               (list-ref (list-ref letters i) 0)
               (list-ref (list-ref letters i) 1)
               0
               GL_RGBA
               GL_UNSIGNED_BYTE
               (list-ref (list-ref letters i) 2))
